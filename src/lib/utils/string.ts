export function slugify(str: string, maxLength = 50) {
	...
	// Mapeo de caracteres especiales por idioma (español)
	// const locale = {
	//   "%": "por ciento",
	//   "&": "y",
	//   "<": "menor que",
	...
	//   "♥": "love"
	// }
	function clean(slug: string) {
		const guionIndex = slug.lastIndexOf('-')
		return slug.slice(0, guionIndex)
		// .replace(/\$/, '')
	}
	...
	const slug = str
		...
		.trim()
		// Reemplazo de espacios en blanco por el carácter de reemplazo
		.replace(/\s+/g, '-')
		// .slice(0, 60)
		.replace(/\--+/, '')
		.split('-')
		.filter((char) => !['para', 'con', 'de', 'y'].includes(char))
		.join('-')
		.slice(0, 60)
